...
for (let item of users) {
  ...
  SELECT id INTO v_user_id FROM public.profiles WHERE lower(email) = '${email}' LIMIT 1;
  IF v_user_id IS NULL THEN
    SELECT id INTO v_user_id FROM auth.users WHERE lower(email) = '${email}' LIMIT 1;
  END IF;
...
console.log('SQL v2 generado con éxito en scripts/import_all_127_users.sql!');
